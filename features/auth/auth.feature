...
Feature: Authentication API
  ...
  Scenario: Login fails with invalid password
    When I send a POST request to "/auth/login" with email "john@mail.com" and password "wrongpassword"
    Then the response status should be 401

  @regression
  Scenario: Login fails with non-existent email
    When I send a POST request to "/auth/login" with email "nobody@nowhere.com" and password "doesntmatter"
    Then the response status should be 401

  @regression
  Scenario: Login fails when email is empty
    When I send a POST request to "/auth/login" with email "" and password "changeme"
    Then the response status should be 401
  ...
  Scenario: Fetch profile without an access token
    When I send a GET request to "/auth/profile" without a token
    Then the response status should be 401

  @regression
  Scenario: Fetch profile with an invalid access token
    When I send a GET request to "/auth/profile" with token "invalid.jwt.token"
    Then the response status should be 401
  ...
  Scenario: Refresh access token with a valid refresh token
    Given I am logged in with valid credentials
    When I send a POST request to "/auth/refresh-token" with the stored refresh token
    Then the response status should be 201
    And the response body should contain an "access_token"
    And the response body should contain a "refresh_token"
  ...
  Scenario: Refresh token fails with an invalid refresh token
    When I send a POST request to "/auth/refresh-token" with refreshToken "bad.token.value"
    Then the response status should be 401

  @regression
  Scenario: Refresh token fails when refresh token is missing
    When I send a POST request to "/auth/refresh-token" with an empty refresh token
